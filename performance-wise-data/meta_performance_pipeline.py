...
def process_performance_metrics(df: pd.DataFrame, creative_cache: Dict[str, str]) -> pd.DataFrame:
    ...
    PURCHASE_ALIASES = ["purchase", "offsite_conversion.fb_pixel_purchase"]
    ATC_ALIASES = ["add_to_cart", "offsite_conversion.fb_pixel_add_to_cart"]
    LPV_ALIASES = ["landing_page_view", "offsite_conversion.fb_pixel_landing_page_view"]
    ...
    for _, row in df.iterrows():
        ...
        purchases = sum(get_action_value(actions, al) for al in PURCHASE_ALIASES)
        atcs = sum(get_action_value(actions, al) for al in ATC_ALIASES)
        lp_views = sum(get_action_value(actions, al) for al in LPV_ALIASES)
        revenue = sum(get_action_value(action_values, al) for al in PURCHASE_ALIASES)
    ...
    # Re-enforce exact requested column order
    cols = [
        ...
        "c2v_ratio", "cost_per_result", "roas", "cvr", "add_to_cart", "purchases", "revenue"
    ]
    return res_df[cols]
